...
def render_evidence_board_studio():
 ...
 if st.button("🔎 Discover Story + Pictures",type="primary",use_container_width=True):
  if not url.strip():st.warning("Paste a public URL first.")
   ...
   try:
    with st.spinner("Analyzing public page and collecting available images..."): story=fetch_story(url,max_images=12); found=download_images(story,max_downloads=8); beats=make_story_beats(story,count=max(2,min(6,len(found) or 4)))
 ...
 selected=selected[:6]; pos=positions(len(selected)); items=[]; beats=[]; st.subheader("4. Evidence placement + narration"); auto=st.session_state.get("eb_beats",[]); cols=st.columns(min(3,len(selected)))
 for i,(name,im,src) in enumerate(selected):
  x,y=pos[i]
  with cols[i%len(cols)]:
   st.image(im,width=120,caption=name); x=st.slider("X",.05,.95,x,.01,key=f"eb_x_{i}"); y=st.slider("Y",.10,.90,y,.01,key=f"eb_y_{i}"); rot=st.slider("Tilt",-20,20,-8 if i%2==0 else 8,key=f"eb_rot_{i}"); scale=st.slider("Size",.10,.40,.22,.01,key=f"eb_scale_{i}"); fallback=f"Evidence item {i+1} reveals another part of the story."; line=st.text_area("Narration",value=(auto[i] if i<len(auto) else fallback),height=90,key=f"eb_line_{i}")
