...
def extract_video_frames(video_path,output_dir=None,max_frames=24):
    ...
    for i in range(count):
        ...
        r=subprocess.run([ff,"-hide_banner","-loglevel","error","-y","-ss",str(t),"-i",str(video_path),"-frames:v","1","-q:v","3",str(p)],capture_output=True,text=True)
        if r.returncode==0 and p.exists() and p.stat().st_size>0: paths.append({"path":str(p),"timestamp":round(t,2)})
    if not paths: raise RuntimeError("FFmpeg could not decode any frames from this video. Try an MP4/H.264 file.")
    return {"duration":duration,"frames":paths}

def _variance(img):
    stat=ImageStat.Stat(img.convert("L")); return sum(stat.var)/max(1,len(stat.var))
def _edge_score(img):
    gray=img.convert("L").resize((160,90)).filter(ImageFilter.FIND_EDGES); stat=ImageStat.Stat(gray)
    return min(1.0,float(stat.mean[0])/80.0)
def _skin_score(img):
...
def _text_score(img):
    small=img.convert("L").resize((160,90)); e=small.filter(ImageFilter.FIND_EDGES); stat=ImageStat.Stat(e)
    edge=float(stat.mean[0])/255.0; var=_variance(img)
...
def score_frame(img,previous=None):
    ...
    if previous is not None:
        ...
        diff=sum(abs(x-y) for x,y in zip(a.getdata(),b.getdata()))/(64*36*255); scene=min(1.0,diff*3.2)
    distinctive=min(1.0,visual*.42+texture*.33+scene*.25); objects=min(1.0,visual*.55+texture*.45); location=min(1.0,visual*.35+(1-people)*.25+texture*.40)
...
def rank_evidence_frames(frame_records,top_n=10): return sorted(frame_records,key=lambda x:x.get("scores",{}).get("overall",0),reverse=True)[:top_n]
def analyze_video_frames(video_path,max_frames=24,top_n=10):
    ...
    for f in extracted["frames"]:
        img=Image.open(f["path"]).convert("RGB"); scores=score_frame(img,previous); previous=img; records.append({**f,"scores":scores})
...
def extract_video_audio(video_path,output_path=None):
    ...
    r=subprocess.run([ff,"-hide_banner","-loglevel","error","-y","-i",str(video_path),"-vn","-ac","1","-ar","16000","-c:a","pcm_s16le",str(out)],capture_output=True,text=True)
...
def transcribe_video(video_path,model_size="tiny"):
    ...
    model=WhisperModel(model_size,device="cpu",compute_type="int8"); segments,_=model.transcribe(audio,beam_size=1); data=[{"start":round(s.start,2),"end":round(s.end,2),"text":s.text.strip()} for s in segments]
