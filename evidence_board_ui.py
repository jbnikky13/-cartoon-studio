...
def positions(n):
    p={1:[(.5,.5)],2:[(.32,.42),(.68,.55)],3:[(.22,.35),(.52,.55),(.80,.32)],4:[(.20,.30),(.45,.60),(.68,.28),(.85,.58)],5:[(.16,.30),(.38,.55),(.58,.28),(.78,.55),(.90,.25)],6:[(.14,.28),(.32,.58),(.50,.25),(.68,.58),(.84,.28),(.92,.62)]}
    return p.get(n,[(.1+.8*i/max(1,n-1),.4) for i in range(n)])
...
def render_evidence_board_studio():
    st.header("🕵️ Evidence Board / Discovery Story")
    st.caption("Paste a public story, discover evidence, choose a visual investigation style, edit the narration, and render a documentary-style board.")
    st.subheader("1. Discovery Story")
    url=st.text_input("Public article / webpage URL",placeholder="https://example.com/story",key="eb_url")
        ...
        if not url.strip(): st.warning("Paste a public URL first.")
            ...
            try:
                with st.spinner("Analyzing public page and collecting available images..."):
                    story=fetch_story(url,max_images=12)
                    found=download_images(story,max_downloads=8)
                    beats=make_story_beats(story,count=max(2,min(6,len(found) or 4)))
                st.session_state.update(eb_story=story,eb_found=found,eb_beats=beats)
                st.success(f"Draft created: {story['title']} — {len(found)} usable pictures")
    ...
    if story:
        ...
        st.caption(story.get("description","")[:600]); st.caption(f"Source: {story.get('url','')}")
        with st.expander("📖 Extracted story text"): st.write(story.get("text","")[:16000])
    st.subheader("2. Investigation background")
    style=st.selectbox("Board style",BG_STYLES,key="eb_style")
    board_bg=make_board_background(style); st.image(board_bg,caption=style,width=430)
    custom=st.file_uploader("Or upload your own background",type=["png","jpg","jpeg"],key="eb_bg")
    if custom: board_bg=Image.open(custom).convert("RGB").resize((1280,720))
    st.subheader("3. Evidence")
    source=st.radio("Evidence source",["Discovered pictures","Upload manually"],horizontal=True,key="eb_source")
    selected=[]; credits=[]
    if source=="Discovered pictures" and found:
        st.caption("Only use images you have permission to publish. Source URLs are retained for attribution/review.")
        ...
        for i,v in enumerate(found):
            ...
            with cols[i%len(cols)]:
                ...
                if st.checkbox("Use",value=i<min(6,len(found)),key=f"eb_use_{i}"):
                    selected.append((f"evidence_{i+1}.jpg",im,v.get("url",""))); credits.append(v.get("url",""))
    else:
    ...
    if len(selected)<2:
        st.info("Choose at least 2 evidence items to continue."); return
    selected=selected[:6]; pos=positions(len(selected)); items=[]; beats=[]
    st.subheader("4. Evidence placement + narration")
    auto=st.session_state.get("eb_beats",[])
    cols=st.columns(min(3,len(selected)))
    for i,(name,im,src) in enumerate(selected):
        x,y=pos[i]
        with cols[i%len(cols)]:
            st.image(im,width=120,caption=name)
            x=st.slider("X",.05,.95,x,.01,key=f"eb_x_{i}"); y=st.slider("Y",.10,.90,y,.01,key=f"eb_y_{i}")
            rot=st.slider("Tilt",-20,20,-8 if i%2==0 else 8,key=f"eb_rot_{i}"); scale=st.slider("Size",.10,.40,.22,.01,key=f"eb_scale_{i}")
            fallback=f"Evidence item {i+1} reveals another part of the story."
            line=st.text_area("Narration",value=(auto[i] if i<len(auto) else fallback),height=90,key=f"eb_line_{i}")
