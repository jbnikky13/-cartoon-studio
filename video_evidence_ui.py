...
def _run_analysis(video_path):
    result=analyze_video_frames(str(video_path),max_frames=12,top_n=8)
    if not result.get("frames"): raise RuntimeError("No video frames could be extracted. Check the video format and FFmpeg availability.")
    return result

def _normalize_scores(scores):
    return {"scene_change":scores.get("scene_change",0),"faces_people":scores.get("people_faces",scores.get("faces_people",0)),"documents_text":scores.get("documents_text",0),"objects":scores.get("objects",0),"locations":scores.get("locations",0),"visual_distinctiveness":scores.get("visually_distinctive",scores.get("visual_distinctiveness",0)),"overall_relevance":scores.get("overall",scores.get("overall_relevance",0))}

def render_video_evidence_panel():
    st.subheader("🎥 Video → Story + Evidence"); st.caption("Upload a video or paste a public video page/direct-file URL. Cartoon Studio extracts frames, scores evidence, and can transcribe available speech.")
    ...
    if st.button("🧠 Analyze Video",type="primary",use_container_width=True,key="eb_analyze_video"):
        try:
            ...
            progress=st.progress(0.0,text="Starting analysis..."); progress.progress(.15,text="Extracting and scoring visual frames..."); result=_run_analysis(vp); progress.progress(.65,text="Analyzing available speech..."); transcript=transcribe_video(str(vp),model_size="tiny"); progress.progress(1.0,text=f"Finished — analyzed {len(result['frames'])} frames")
    ...
    with st.expander("📝 Extracted story/audio transcript",expanded=False):
        ...
        else: st.info("No transcript was produced. Visual evidence still works; speech-to-text is optional.")
    top_n=st.slider("Number of evidence frames",2,min(6,len(ranked)),min(6,len(ranked)),key="eb_video_topn"); st.markdown("**⭐ Ranked evidence frames**"); selected=[]; cols=st.columns(min(3,len(ranked)))
    for i,rec in enumerate(ranked[:12]):
        with cols[i%len(cols)]:
            s=_normalize_scores(rec["scores"]); st.image(rec["path"],caption=f"#{i+1} • {rec['timestamp']}s • Score {s['overall_relevance']}",width=180); st.caption(f"Scene {s['scene_change']} · People {s['faces_people']} · Text {s['documents_text']} · Objects {s['objects']} · Location {s['locations']}")
    ...
    if selected and st.button("📌 Send Selected Frames to Evidence Board",use_container_width=True,key="eb_send_video_frames"):
        ...
        st.session_state.eb_video_selected=payload
        st.session_state.eb_found=payload
        st.session_state.eb_story={"title":"Video Evidence Story","description":"Evidence frames selected from the analyzed video.","url":st.session_state.get("eb_video_source_url","")}
        st.success(f"Sent {len(payload)} evidence frames to the board. Open the Evidence Board and choose Discovered pictures.")
