"""Lightweight visual evidence scoring for sampled video frames."""
...
def _cv(img):
 ...
 lines=cv2.HoughLinesP(cv2.Canny(gray,80,180),1,np.pi/180,45,minLineLength=max(20,gray.shape[1]//8),maxLineGap=8)
...
def analyze_frame(image_path,previous_image=None):
 ...
 if prev:
  import numpy as np
  scene=min(100,float(np.mean(abs(np.asarray(img,dtype=float)-np.asarray(prev,dtype=float))))*2)
