"""Memory-conscious video evidence analyzer: sample, downscale, score, transcribe."""
...
def extract_video_frames(video_path,output_dir=None,max_frames=12):
    ...
    count=max(1,min(max_frames,int(math.ceil(duration/5))))
    paths=[]
    for i in range(count):
        t=0 if count==1 else duration*i/(count-1); p=out/f"evidence_{i+1:02d}.jpg"
        r=subprocess.run([ff,"-hide_banner","-loglevel","error","-y","-ss",str(t),"-i",str(video_path),"-frames:v","1","-vf",f"scale={FRAME_WIDTH}:-2","-q:v","5",str(p)],capture_output=True,text=True)
...
def _edge_score(img):
    gray=img.convert("L").resize((160,90)).filter(ImageFilter.FIND_EDGES); return min(1.0,float(ImageStat.Stat(gray).mean[0])/80.0)
def _skin_score(img):
...
def _text_score(img):
    small=img.convert("L").resize((160,90)); edge=float(ImageStat.Stat(small.filter(ImageFilter.FIND_EDGES)).mean[0])/255.0; var=_variance(img)
...
def score_frame(img,previous=None):
    ...
    if previous is not None:
        a=img.resize((64,36)).convert("L"); b=previous.resize((64,36)).convert("L"); diff=sum(abs(x-y) for x,y in zip(a.getdata(),b.getdata()))/(64*36*255); scene=min(1.0,diff*3.2)
...
def rank_evidence_frames(frame_records,top_n=8): return sorted(frame_records,key=lambda x:x.get("scores",{}).get("overall",0),reverse=True)[:top_n]
def analyze_video_frames(video_path,max_frames=12,top_n=8):
