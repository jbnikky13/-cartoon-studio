...
def _write_text(work,name,text): p=Path(work)/name; p.write_text(str(text or ""),encoding="utf-8"); return p
...
def finalize_discovery_export(master_path,output_path,beats,items,aspect="16:9",subtitles=True,labels=True,title="DISCOVERY STORY",progress_cb=None):
    ...
    try:
        ...
        sizes={"16:9":(1280,720),"9:16":(720,1280),"1:1":(900,900)}; ow,oh=sizes.get(aspect,sizes["16:9"]); filters=[f"scale={ow}:{oh}:force_original_aspect_ratio=increase",f"crop={ow}:{oh}"]; t=0.0
        ...
        for i,beat in enumerate(beats):
            ...
            if subtitles and text:
                p=_write_text(work,f"subtitle_{i}.txt",text); filters.append(f"drawtext=fontfile='{_esc_filter_path(font)}':textfile='{_esc_filter_path(p)}':fontcolor=white:fontsize=26:borderw=3:bordercolor=black:box=1:boxcolor=black@0.72:boxborderw=14:x=(w-text_w)/2:y=h-text_h-42:enable='between(t\,{start:.3f}\,{end:.3f})'")
            if labels:
                idx=getattr(beat,"item_index",i) if hasattr(beat,"item_index") else i; name=getattr(items[idx],"name",f"Evidence {idx+1}") if 0<=idx<len(items) else f"Evidence {idx+1}"; p=_write_text(work,f"label_{i}.txt",_clean_text(f"EVIDENCE {idx+1}  •  {name}",80)); filters.append(f"drawtext=fontfile='{_esc_filter_path(bold)}':textfile='{_esc_filter_path(p)}':fontcolor=white:fontsize=22:borderw=2:bordercolor=black:box=1:boxcolor=black@0.62:boxborderw=10:x=34:y=94:enable='between(t\,{start:.3f}\,{end:.3f})'")
            t=end
        if title:
